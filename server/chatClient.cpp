        ...
        else
        {
            ...
            else if(header->message_type == SerarchUser_MESSAGE)
            {
             ...
             else if(header->message_type == transAudioToServer)
             {
                ...
                acceptAudioMessage(ReceiveName);
             }
...
void ChatClient::acceptAudioMessage(std::string receiveName)
{
    int* size = new int;
    asio::async_read(*clientSocket , asio::buffer(size , sizeof(int)) , [this , size , receiveName](asio::error_code ec, size_t length){
        if(ec)
            return;
        if(*size == 0)
        {
            std::cout << "结束音频传输";
            std::cout << "删除了" << audioChatCurrentName << "的socket\n";
            for(auto& tmp : client)
            {
                if(tmp->audioChatCurrentName == receiveName)
                {
        ...
        asio::async_read(*clientSocket , asio::buffer(data , *size) , [this , data , receiveName , size](asio::error_code ec, size_t length){
            if(ec)
                return;
            std::cout << "接收到：" << *size << "音频数据\n";
            for(auto& tmp : client)
            {
                if(tmp->audioChatCurrentName == receiveName)
                {
